...
{
    public partial class VScrollBarEx : VScrollBar
    {
        #region  字段
        ...
        public new int Value
        {
            ...
            set
            {
                if (this.value != value)
                {
                    ...
                    _barRectangle.Y = (this.maximum == this.minimum || this.maximum == this.LargeChange) ? 0 : value * (Height - (_barRectangle.Height > SMALLLENGTH ?
                        _barRectangle.Height : SMALLLENGTH)) / (this.maximum - this.minimum - this.LargeChange);
        ...
        public new int Maximum
        {
            ...
            set
            {
                if (this.maximum != value)
                {
                    ...
                    this.maximum = value;
                    int barHeight = maximum == minimum ? SMALLLENGTH : LargeChange * Height / (maximum - minimum);
                    _barRectangle.Height = barHeight >= SMALLLENGTH ? barHeight : SMALLLENGTH;
                    this.Invalidate();
                }
            }
        }
        ...
        public VScrollBarEx()
        {
            ...
            SetStyle(ControlStyles.UserPaint | ControlStyles.UserMouse | ControlStyles.AllPaintingInWmPaint |
                ControlStyles.OptimizedDoubleBuffer | ControlStyles.StandardClick | ControlStyles.SupportsTransparentBackColor, true);
            
			_barRectangle = new Rectangle(0, 0, Width, SMALLLENGTH);
        }
        ...
        public void ResizeBarRectangle()
        {
            try
            {
                if (this.Visible)
                {
                    ...
                    int barHeight = maximum == minimum ? SMALLLENGTH : LargeChange * Height / (maximum - minimum);
                    _barRectangle.Height = barHeight >= SMALLLENGTH ? barHeight : SMALLLENGTH;
                    _barRectangle.Y = (maximum == minimum || maximum == LargeChange) ? 0 : value * (Height - (_barRectangle.Height > SMALLLENGTH ?
                            _barRectangle.Height : SMALLLENGTH)) / (maximum - minimum - LargeChange);
                    _barRectangle.Width = Width;
                    this.ResumeLayout();
                    this.Invalidate();
                }
            }
